...
router.get('/attachPlan',isLoggedIn,teacher,function(req,res){
  /* var companyId = req.user.companyId
   Class1.find({companyId:companyId},function(err,focs){
   ...
   res.render('teacherExam/plan',{ arr1:arr1,arr2:arr2,class1:class1, user:user,icon:icon, pro:pro, subject:subject,subjectCode:subjectCode, grade:grade,successMsg: successMsg,errorMsg:errorMsg, noMessages: !successMsg,noMessages2:!errorMsg})
...
router.post('/attachPlan',upload.single('file'),isLoggedIn,teacher, function(req,res){
  var pro = req.user
  var day = req.body.day
  var m2 = moment()
  var mformat = m2.format('L')
...
if (errors) {
 ...
 /* res.render('teacherExam/assgt',{errors:req.session.errors,  arr:arr,pro:pro})
  ...
  req.flash('danger', req.session.errors[0].msg);
        ...
        res.redirect('/teacher/attachPlan');
}
...
let photo = docs[i].photo*/
    ...
    req.flash('success', 'Lesson Plan Posted Successfully!');
...
router.get('/typeFolderMaterial/:id',isLoggedIn,teacher,function(req,res){
  ...
  ////////////////xxxxxxxxxxxxxx
  router.get('/typeFolderPlans/:id',isLoggedIn,teacher,function(req,res){
    var id = req.params.id
    var term = req.user.term
    var m = moment()
    var mformat = m.format('L')
    ...
    TeacherSub.findById(id,function(err,doc){
      ...
      Learn.find({subjectCode:subjectCode,term:term,year:year,class1:class1,type:"lesson plan"},function(err,locs){
        ...
        res.render('repositoryT/filesP',{listX:locs,pro:pro,teacherSubId:teacherSubId,
          subject:subject,id:id,class1:class1,id2:id2,
        })
...
let contentType = files[0].contentType
  ...
  //plans
  router.get('/openPlans/:id',(req,res)=>{
    var fileId = req.params.id
      const bucket = new mongodb.GridFSBucket(conn.db,{ bucketName: 'uploads' });
      gfs.files.find({_id: mongodb.ObjectId(fileId)}).toArray((err, files) => {
  ...
  router.get('/downloadPlans/:id',(req,res)=>{
    var fileId = req.params.id
